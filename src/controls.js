...
// The thirteen spaces of TM59:2026 Appendix E, generated by
...
/**
 * A day as twenty-four hourly fractions, each a share of the control's peak.
 *
 * `Profile` is the desk's other shape control and it cannot say this. A band
 * has a start and an end, so it can say *when* a room is used and nothing
 * whatever about how much. CIBSE TM59's bedroom stands above 0.7 in every hour
 * of the day — which is why all 3672 of a summer's hours are occupied hours in
 * one — and it does not stand at one figure through it, so a band forced onto
 * that profile would have to choose a single level and letter it as the whole
 * day. The four gaps between what TM59 asks for and what this strip offers are
 * set out in the feature's research; this kind closes the third of them.
 *
 * The value is the canonical text rather than the array, for the reason `Days`
 * above is text and it is worth restating because it is the trap that would be
 * found late: every parameter on this desk is a scalar and four separate
 * mechanisms rely on it, the worst of them `revert`'s
 * `Object.assign(params, DEFAULT_PARAMETERS)`. `Object.freeze` is shallow, so
 * an array default would be *aliased* into live `params` by that assign, and
 * the first edit would corrupt `DEFAULT_PARAMETERS` for the rest of the
 * session. That object is also the permalink's `DEFAULTS_BY_VERSION.v1`, so
 * the link format itself would drift, with no symptom at all until a shared
 * link came back describing a different building. `parsePattern` and
 * `serializePattern` below are the boundary, declared beside `parseHolidays`
 * and `serializeHolidays` because the rules for what a control can hold belong
 * with the declaration and every codec reads them from there.
 *
 * Two things this kind deliberately cannot do. It is **not sweepable**: a study
 * samples between `min` and `max` along a `step`, and a pattern has no face to
 * sample along — twenty-four numbers is not a position, it is a shape. And it
 * carries **no landmarks**: `readLandmarks`'s four rules are every one of them
 * about a numeric face and its step grid, and nobody publishes a band for the
 * shape of a daily profile. Both absences are the honest answer rather than a
 * gap, on the same terms as the em dash on the drawing.
 */
export class Pattern extends Control {
  constructor({ key, label, hours, digits = 2, note = null, needs = null, when = null }) {
    // `when` is handed to the base class rather than quietly dropped, so that
    // `assertHideable` at the foot of this file is what refuses it. The console
    // cannot withdraw a pattern until `buildControl` learns to read `shown()`,
    // and a declaration accepted and then ignored is precisely the silent
    // breakage that assertion exists to turn into a throw at mount.
    super({ key, label, value: serializePattern(hours, digits), note, needs, when });
    this.kind = 'pattern';
    // The default, as a declaration. What reaches `params` is the text above;
    // nothing downstream ever holds this array, and it is kept because a
    // declaration that can only be read back through its own parser is a
    // declaration you cannot check by eye.
    this.hours = Object.freeze([...hours]);
    this.digits = digits;
    // A fraction the canonical text cannot carry is a declaration disagreeing
    // with itself. The console rounds a typed value to `digits` before it
    // commits, exactly as a margin number is rounded to its step's own
    // decimals, so a default of 0.189 on a two-decimal control would be
    // lettered, committed and written into the IDF as 0.19 while this array
    // went on saying 0.189 and nothing on the page reported the difference.
    // TM59's own fractions are divisions of Table E.1's absolute watts —
    // 85 / 450 is 0.1889 — so the hazard is live rather than hypothetical, and
    // it throws here at module load rather than degrading at the first edit.
    hours.forEach((h, i) => {
      if (Number(h.toFixed(digits)) !== h) {
        throw new Error(
          `${key} starts at ${h} in the hour from ${String(i).padStart(2, '0')}:00, `
          + `which ${digits} decimal${digits === 1 ? '' : 's'} cannot carry`,
        );
      }
    });
  ...
  /**
   * The shape in one line, which is all the margin has room for.
   *
   * A flat pattern says so — a reader who has not opened the fold needs to know
   * the day is level before they need to know at what. Otherwise it letters the
   * two ends and, where any hour is zero, how many hours stand above it: that
   * count is the pattern's occupied hours, and it is the number every criterion
   * read off this desk takes as its denominator, so it is worth having in the
   * margin rather than only in the fold.
   */
  format(v) {
    const hours = parsePattern(v);
    const written = (n) => n.toFixed(this.digits);
    const low = Math.min(...hours);
    const high = Math.max(...hours);
    if (low === high) return `Flat ${written(low)}`;
    const standing = hours.filter((h) => h > 0).length;
    return standing === hours.length
      ? `${written(low)}–${written(high)}`
      : `${written(low)}–${written(high)} · ${standing} h`;
  }
}
...
/** Hours in a day, and therefore fields in a pattern. */
export const PATTERN_HOURS = 24;

/**
 * A plain decimal and nothing else, which is narrower than `Number`'s grammar
 * on purpose and for the reason the link codec gives above its own copy:
 * `Number('')` is 0, `Number(' ')` is 0 and `Number('0x18')` is 24, so a
 * truncated field or a stray space would be read as a real occupancy rather
 * than refused. No exponent either — a canonical text has to be a thing a
 * reader can check against the fold beside it, and `1e-3` is not.
 */
const FRACTION = /^-?\d+(\.\d+)?$/;
 ...
 * as a sentence and `refuses` returns it as a clause, the way a link and a
 * preset already share `refuses` itself. Restated twice they would drift, and
 * the reader would be told two different things about one field.
 */
function patternFault(text) {
  if (typeof text !== 'string') return 'is not text';
  const fields = text.split(',');
  if (fields.length !== PATTERN_HOURS) {
    return `has ${fields.length} field${fields.length === 1 ? '' : 's'} rather than the ${PATTERN_HOURS} hours of a day`;
  }
  for (let h = 0; h < PATTERN_HOURS; h += 1) {
    const at = `${String(h).padStart(2, '0')}:00`;
    if (!FRACTION.test(fields[h])) return `reads "${fields[h]}" at ${at}, which is not a number`;
    const value = Number(fields[h]);
...
/**
 * Canonical text to twenty-four fractions.
 *
 * Refused whole, never half-read: a pattern with twenty-three hours in it is
 * not a day with an hour missing, it is a text this desk cannot say what to do
 * with, and reading the twenty-three would be the silent repair the rest of
 * this file exists not to make. Same rule as `12abc` in a margin field and a
 * malformed link.
 */
export function parsePattern(text) {
  const fault = patternFault(text);
  if (fault) throw new Error(`a pattern ${fault}`);
  return Object.freeze(text.split(',').map(Number));
}

/**
 * Twenty-four fractions to canonical text, at the pattern's own precision.
 *
 * `parsePattern(serializePattern(hours, digits))` is `hours` rounded to
 * `digits`, and the round trip is exact in the direction that matters — a
 * `Pattern` declares its own hours at its own precision and throws at module
 * load where the two disagree, so the text a control was built from reads back
 * as the array it was built from. Going the other way,
 * `serializePattern(parsePattern(text), digits)` returns `text` unchanged for
 * any text already written to exactly `digits` decimals, which is every text
 * this module or the console ever produces.
 */
export function serializePattern(hours, digits = 2) {
  if (!Array.isArray(hours)) throw new Error('a pattern is written from an array of hourly fractions');
  if (hours.length !== PATTERN_HOURS) {
    throw new Error(`a pattern is ${PATTERN_HOURS} hourly fractions, and this one is ${hours.length}`);
  }
  return hours
    .map((h, i) => {
      const at = `${String(i).padStart(2, '0')}:00`;
...
export function refuses(control, value) {
  ...
  // A pattern is text too, and it is above the numeric gate for the same
  // reason — but it validates rather than throwing, because unlike a month mask
  // and a day list it *is* reachable by both of the callers this function has:
  // TM59's prescribed profiles arrive as a standard's specification, and any
  // link that carries one has to be checked against the same rules. Refused
  // whole, with the first field that is wrong named, because there is no
  // half-reading of a pattern with twenty-three hours in it.
  if (control.kind === 'pattern') {
    return patternFault(value);
  }
 ...
 * See `Rooflights` in `aperture.js` for the severe it refuses. The tolerance is
 * quoted in metres in both systems, as `tooShallow` quotes it: it is the engine's
 * constant, not a reading of the reader's controls.
 */
const SKY_REASONS = Object.freeze({
  noRoof: 'Needs a roof with an outside to cut a rooflight into.',
  tooThin: `Under ${COINCIDENT} m deep, EnergyPlus deletes these rooflights: raise the ratio or run fewer.`,
});
...
/**
 * Which of the Gains strip's two models is in force, which is the same
 * arrangement the Air strip carries one channel up.
 *
 * The desk's own model is a density and a band: so many square metres to the
 * person, so many watts to the square metre, all of it on one occupancy
 * profile shared by the people, the lights and the equipment. CIBSE TM59
 * prescribes the other kind entirely — *this* room holds so many people and so
 * many watts of equipment, on three daily shapes of its own — and the two cannot be
 * expressed by one set of controls: an absolute count needs no floor area,
 * which is what lets a standard prescribe it without reaching into Massing,
 * and a fraction-per-hour is a shape a from/to band cannot hold.
 *
 * `AS_DRAWN` is exported rather than spelled out three times, because three
 * things test against it — these two predicates, `applyGains`, and the
 * register's own specification — and a literal repeated across three files is
 * how a later rename becomes a strip that silently writes the wrong model
 * while every one of the three goes on looking right.
 *
 * **The default is what keeps every link minted before this feature resolving
 * unchanged.** At `As drawn` the appliers write exactly what they wrote
 * before: one shared band schedule, `Area/Person` people and `Watts/Area`
 * equipment. Under delta encoding an old link simply omits all six new keys and
 * takes their defaults, so the document it decodes to is byte-identical to the
 * one it decoded to yesterday. `LINK_VERSION` does not move and `MIGRATIONS`
 * stays empty — the free pass a new control gets, which a *changed default*
 * would not.
 */
export const AS_DRAWN = 'As drawn';
const asDrawn = (p) => p.roomType === AS_DRAWN;
const prescribed = (p) => p.roomType !== AS_DRAWN;

/**
 * A day at full peak in every one of its hours.
 *
 * The three patterns start flat rather than at any published profile, and that
 * is the honest starting position: a pattern is a multiplier on the peak beside
 * it, so a flat day is the setting at which the pattern says nothing and the
 * absolute figures say everything. Every published shape arrives with the room
 * type, through the register, carrying the sentence of Table E.1 it was divided
 * out of. A shipped default that was one of the thirteen would be this desk
 * asserting a space it has not been told it is.
 */
const FLAT_DAY = Object.freeze(Array.from({ length: PATTERN_HOURS }, () => 1));

/**
 * The spaces TM59 prescribes a setup for, read off the profile library rather
 * than restated here.
 *
 * These were once five names typed into this file, because when the control was
 * declared the Appendix E tables had not been transcribed and a shorter honest
 * list beat five invented ones. The library exists now, and the moment it did
 * the two declarations disagreed: this file offered `Bedroom` and
 * `src/tm59.data.js` carried `Single bedroom` and `Double bedroom`, so the
 * register's preset named a space the selector could not offer and `schemes.js`
 * threw at module load. The page did not mount.
 *
 * That is the failure `SKY_MAX` and `PANE_MAX` are each read off a control to
 * avoid, in the same direction: a list repeated in two places is one edit away
 * from a silent disagreement, and this one was not silent only because the
 * register happens to assert its own room type against the selector's options.
 * So the profile library is the vocabulary and the selector reads it. A space
 * the desk can offer is exactly a space the library can profile, by
 * construction rather than by inspection, and adding a fourteenth to Table E.1
...
export const CHANNELS = Object.freeze([
  ...
  new Channel({
    ...
    controls: [
      // At the head of the strip because it is the strip's model switch, in the
      // position `airModel` holds one channel down and for the same reason: it
      // is the only control that brings the withdrawn half of this strip back.
      new Selector({
        key: 'roomType', label: 'Room type', value: AS_DRAWN,
        options: [
      ...
      // `UNTOUCHABLE` to a preset. Fractional because the cross-check
      // transcription of Table E.2 encodes the two-bedroom kitchen as half a
      // person where Table E.1 reads it as two people at 25 % — the two say the
      // same thing and this face can hold either, so the register is free to
      // letter whichever arithmetic it is citing.
      //
      // One person is not a published default. It is where the face starts, and
      // it starts with the home office because that is the one space the
      ...
      // makes `refuses` demand a whole number of a preset or a link. A published
      // peak this face cannot reach is refused out loud rather than clamped: the
      ...
      // 450 W absolute load, and hold one live over an equipment peak of zero —
      ...
      // 23:00 whatever the room is doing, so a single schedule shared by the
      // people, the lights and the equipment cannot carry the method however it
      // is drawn.
      //
      // Three decimals rather than two, on all three, and it is not a
      // preference. The fractions are divisions of Table E.1's absolute watts,
      // and Table E.2's own two-decimal printing of them does not multiply back:
      // 85 / 450 is 0.1889 and E.2's 0.19 comes back as 85.5 W. The primary
      // statement is the watts, so the canonical text is carried to the
      // precision the division actually has, and `Pattern` throws at module load
      // for a declaration its own digits cannot hold.
      //
      // Dimmed rather than withdrawn, unlike the two scales above, because the
      // console cannot yet withdraw a pattern — `assertHideable` is what says so,
      // and it says it as a throw the moment anyone declares `when` here.
      new Pattern({
        key: 'occPattern', label: 'Occupancy pattern', hours: FLAT_DAY, digits: 3,
        needs: prescribed,
        note: 'A fraction of the peak, hour by hour. The band below can only say when the room is used: it cannot hold a bedroom that stands above 0.7 in every hour of the day and at a different level in each part of it.',
      }),
      new Pattern({
        key: 'equipPattern', label: 'Equipment pattern', hours: FLAT_DAY, digits: 3,
        needs: prescribed,
        note: 'A fraction of the equipment peak beside it, which is where a base gain lives: 19 W under a 150 W peak is 0.127 for the rest of the day.',
      }),
      new Pattern({
        key: 'lightPattern', label: 'Lighting pattern', hours: FLAT_DAY, digits: 3,
        needs: prescribed,
        note: 'On its own band, not the occupied one. This is the profile a shared schedule could not carry.',
      }),
      new Profile({
        key: 'occupied', label: 'Occupied hours', from: 'occFrom', to: 'occTo',
        // The second sentence is a sentence rather than a dimmed row, and that
        // is a shortfall being stated rather than a choice: a `Profile` cannot
        // carry `needs` today, because `buildProfile` reads neither `shown` nor
        // `idle` and a declaration it would ignore is worse than none. Under a
        // named room type the occupancy pattern writes this schedule and the
        // band reaches nothing, so the strip says so until the console can grey
        // it.
        note: 'Writes a Schedule:Compact. Outside the band the gains fall to a tenth. Under a named room type the occupancy pattern writes that schedule instead and this band reaches nothing.',
      }),
      new Selector({
