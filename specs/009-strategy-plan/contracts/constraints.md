...
- keeps every design measured outside the new region in the ledger, stated as ruled out and left out of every figure (FR-050). Nothing is deleted, so widening the region again is free.
...
3. **The grid is region-independent** (SC-017): for a value admitted by both, `designAt` under `Region.EMPTY` and under a narrowing region that still admits it produce byte-identical `params`, so the cache identity is the same string.
