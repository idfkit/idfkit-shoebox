...
Appended by a second `/speckit-converge` run, after four commits changed behaviour on this
branch. T075, T076, T079, T080 and T082 are still open above and are not repeated here.

- [ ] T083 Letter the published source of each figure a named room writes, per FR-021 (partial). `Control.implies` gives `roomType` a second route to the same seven values the TM59 preset writes, and only the preset letters where they came from: its `Spec`s carry 375 to 737 characters of `why` each, with the division that produced every fraction, and `gainsForRoom` in `src/controls.js` returns bare numbers. FR-021 asks that each prescribed value letter its published figure and any arithmetic between the published units and the desk's own, so the conversion can be checked and disagreed with, and the strip's one-line note ("brings that space's published figures with it") is a claim about a citation rather than the citation. Either have the overlay carry its `why` and the Gains strip letter it beside each figure, or say in the specification why a figure arriving by the selector is exempt from a rule a figure arriving by the register obeys. The library already holds the sentences: `RoomProfile.why` is what `cite()` selects from.
- [ ] T084 Supersede T081: the `tm59a` study offer is now gated on the chase pin rather than on `roomType`, per US6/AC1 and FR-003a (contradicts). T081 was written when `studyMetric` in `src/main.js` read `snapshot.roomType !== AS_DRAWN`; it now reads `chased === TM59_PRESET`. That is a better signal — it is the reader saying outright which line they are working against, one press to set and one to drop — and it is the same category of error, because US6/AC1 conditions the curve on a year being attached and nothing else and FR-003a says which criteria are asked is not a setting the desk holds. Do not act on T081 as written; its mechanism has moved. `specs/004-choose-sweep-metric` specifies the settled answer, in which the reader picks the quantity and the desk stops inferring, and deletes this gate along with the other two. Close this by shipping 004, or by writing the chase-pin rule into this specification as a requirement with its reasoning.
- [ ] T085 Assert that the four other routes writing `params` stay consistent with `Control.implies`, per Constitution II and the plan's one-declaration rule (partial). `implies` is consulted at exactly one call site, `commit` in `src/main.js`; `applyPreset`, `revert`, the kept-scheme restore and the link decode all write `params` without asking. Every one of them is consistent today — the TM59 preset writes all seven keys itself and is now asserted to, `revert` resets the whole channel, and a link and a scheme carry explicit values so they faithfully reproduce the desk that was shared — but all four are consistent by arithmetic rather than by construction, and the second control to carry `implies` has to re-establish each by hand. Either add a module-load assertion that a preset writing an implying control also writes everything it implies, or have the routes consult the declaration.
- [ ] T086 Ratify `Control.implies` in `specs/002-tm59-overheating/plan.md` and `data-model.md`, or record why it is out of scope (unrequested). It is a new optional field on the base `Control` class that every control on the desk inherits, added during review to keep `commit` from knowing one control by name, and it is the right depth — but no requirement, plan decision or task called for a general cross-control mechanism, and the plan's Project Structure section still describes `controls.js` as gaining only the `Pattern` kind and six controls. A mechanism the artifacts do not mention is one the next feature will either rediscover or duplicate.
