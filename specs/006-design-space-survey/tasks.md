...
- [ ] T101 Run gate 8 of quickstart.md by hand in a foreground browser at 390 x 640, in both themes, in monochrome and under forced colours, confirming measured, inferred and unsurveyed ground stay distinguishable, per SC-007, SC-009 and FR-048 (missing). **Partly covered already**: driven at **500 x 617**, which is as narrow as Chrome's own minimum window width allows, the survey shows no horizontal overflow, `--survey` engages, the two figures stack, both schedules and the pull fold to blocks with each figure keeping its head, and the relief draws at **full mesh** (848 x 848 over all 144 positions), which is FR-018k. Asserted separately from the source: neither `--cold` nor `--warm` appears anywhere in E-02's stylesheet, `survey.js`, `pull.js` or the relief's shaders, and every pull direction is a word beside its bar. **What is left is 390 itself, monochrome and forced colours**, which need a device or OS settings
